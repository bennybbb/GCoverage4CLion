...
class CoverageThread(private val myProject: Project, private val myBuildDirectory: String, private val toolchain: CPPToolchains.Toolchain, private val myRunner: Runnable?) : Thread() {
    ...
    private val coverageThreadHelper = CoverageThreadHelper();

    init {
        name = "CoverageThread"
    }

//    private fun generateGCDAFor(gcda: File, project: Project, hashSet: HashSet<List<String>>, toolchain1: CPPToolchains.Toolchain): Boolean {
//        try {
...
//                "/dev/zero"
//            }
//            val path = GCovSettings.getInstance().getGCovPathForToolchain(toolchain1)?.gcovPath
//            if (path == null) {
...
//    }
    ...
    override fun run() {
        synchronized(myData) {
            ...
            File(myBuildDirectory).walkTopDown().asSequence().filter {
                val allFilesInDirectory = it
                filterGCDAFilesOutOf(allFilesInDirectory)
            }.toList().forEach {
                val gcdaFile = it
                val isGcdaGenerated = coverageThreadHelper.generateGCDAFor(gcdaFile, myProject, myLinesSaid, toolchain, GCovSettings.getInstance().getGCovPathForToolchain(toolchain)?.gcovPath)
    ...
    private fun filterGCDAFilesOutOf(it: File): Boolean {
        return it.isFile && when {
